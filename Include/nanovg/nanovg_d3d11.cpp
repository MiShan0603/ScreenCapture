...
static struct D3DNVGtexture* D3Dnvg__findTexture(struct D3DNVGcontext* D3D, HANDLE handle)
{
	int i;
	for (i = 0; i < D3D->ntextures; i++)
		if (D3D->textures[i].handle == handle)
			return &D3D->textures[i];
	return NULL;
}
    ...
    for (i = 0; i < D3D->ntextures; i++) {
        if (D3D->textures[i].id == id) {
            if (D3D->textures[i].tex != 0 && (D3D->textures[i].flags & NVG_IMAGE_NODELETE) == 0)
            {
                ...
                D3D_API_RELEASE(D3D->textures[i].resourceView0);
				D3D_API_RELEASE(D3D->textures[i].resourceView1);
            }
...
static int D3Dnvg__renderCreateTexture(void* uptr, int type, int w, int h, int imageFlags, const unsigned char* data)
{
    ...
    D3D_API_3(D3D->pDevice, CreateShaderResourceView, (ID3D11Resource*)tex->tex, &viewDesc, &tex->resourceView0);

    if (data != NULL && texDesc.MipLevels == 0)
    {
        D3D_API_1(D3D->pDeviceContext, GenerateMips, tex->resourceView0);
    }
    ...
    float width, float fringe, float strokeThr)
{
    ...
    if (paint->image != 0) 
    {
		...
		else if (tex->type == NVG_TEXTURE_NV12)
		{
			frag->texType = (tex->flags & NVG_IMAGE_PREMULTIPLIED) ? 3 : 4;
		}
        else
        {
			frag->texType = 2;
...
static void D3Dnvg__setUniforms(struct D3DNVGcontext* D3D, int uniformOffset, int image)
{
    ...
    if (image != 0) 
    {
        ...
        if (tex != NULL)
        {
			if (tex->type == NVG_TEXTURE_NV12)
			{
				ID3D11ShaderResourceView* pSRViews[] = { tex->resourceView0, tex->resourceView1 };
				D3D_API_3(D3D->pDeviceContext, PSSetShaderResources, 0, 2, pSRViews);
			}
			else
    			D3D_API_3(D3D->pDeviceContext, PSSetShaderResources,0, 1, &tex->resourceView0);
        }
        else
        {
...
int nvd3dCreateImageFromHandle(struct NVGcontext* ctx, HANDLE handle, int flags)
{
	...
	struct D3DNVGcontext* D3D = (struct D3DNVGcontext*)nvgInternalParams(ctx)->userPtr;
	struct D3DNVGtexture* tex = D3Dnvg__findTexture(D3D, handle);

	if (tex == NULL)
	{
		// open shared 
		ID3D11Texture2D* texture = NULL;
		if (SUCCEEDED(D3D_API_3(D3D->pDevice, OpenSharedResource, handle, _uuidof(ID3D11Texture2D), (void**)&texture)))
		{
			D3D11_TEXTURE2D_DESC desc;
			texture->GetDesc(&desc);

			tex = D3Dnvg__allocTexture(D3D);

			if(desc.Format == /*DXGI_FORMAT_NV12*/DXGI_FORMAT(103))
				tex->type = NVG_TEXTURE_NV12;
			else
				tex->type = NVG_TEXTURE_RGBA;
			tex->tex = (ID3D11Texture2D*)texture;
			tex->flags = flags;
			...
			if (desc.Format == /*DXGI_FORMAT_NV12*/DXGI_FORMAT(103))
			{
				...
				D3D_API_3(D3D->pDevice, CreateShaderResourceView, (ID3D11Resource*)tex->tex, &SRVDesc, &tex->resourceView0);
				if (tex->resourceView0 == NULL)
				{
					//OutputDebugString(L"resourceView0  null \n");
				}

				SRVDesc.Format = DXGI_FORMAT_R8G8_UNORM;
				D3D_API_3(D3D->pDevice, CreateShaderResourceView, (ID3D11Resource*)tex->tex, &SRVDesc, &tex->resourceView1);
				if (tex->resourceView1 == NULL)
				{
					//OutputDebugString(L"resourceView1  null \n");
				}
			}
			else 
				D3D_API_3(D3D->pDevice, CreateShaderResourceView, (ID3D11Resource*)tex->tex, NULL, &tex->resourceView0);
		} 
	}
