// DXGIScreenCapture.cpp : 定义应用程序的入口点。
//
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_DXGISCREENCAPTURE, szWindowClass, MAX_LOADSTRING);
    ...
    wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_DXGISCREENCAPTURE));
    wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
    wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_DXGISCREENCAPTURE);
    ...
    g_render.Init(g_renderHwnd);

    std::vector<HMONITOR> monitors;
    if (g_montitors[g_curMontitorIdx].hMontitor == NULL)
    {
        for (int index = 0; index < g_montitors.size(); index++)
        {
            if (g_montitors[index].hMontitor != NULL)
                monitors.push_back(g_montitors[index].hMontitor);
        } 
    }
    else
    {
        monitors.push_back(g_montitors[g_curMontitorIdx].hMontitor);
    }

    RECT rect = g_montitors[g_curMontitorIdx].rect;
   
    g_render.Start(monitors, rect);


    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_DXGISCREENCAPTURE));
