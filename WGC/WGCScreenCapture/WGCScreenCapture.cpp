	...
	_In_ int       nCmdShow)
{
	...
	wx.lpszClassName = _T("WGCScreenCaptureClass");
	RegisterClassEx(&wx);

    hInst = hInstance;
	g_hMainWnd = CreateWindowW(wx.lpszClassName, _T("WGCScreenCapture"), WS_OVERLAPPEDWINDOW,
    ...
    g_windowCapture->CaptureMonitor(g_montitors[g_curMontitorIdx].hMontitor);
    g_windowCapture->StartCapture();
